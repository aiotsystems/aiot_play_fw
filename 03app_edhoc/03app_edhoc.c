...
int main(void) {
    ...
    // TODO: Do something with message_1

    if (message_2) {
        res = initiator_parse_message_2(&initiator, message_2, &c_r, &fetched_cred_r, &dummy_ead);
    } else {
        // Error while sending
        return 1;
    }
    ...
    // Verify message_2
    if (!res) {
        res = initiator_verify_message_2(&initiator, &I, &cred_i, &cred_r);
    } else {
      return 1;
    }
    ...
    // TODO: Do something with message_3

    uint8_t app_payload[13] = { 'H', 'e', 'l', 'l', 'o', ',', ' ', 'w', 'o', 'r', 'l', 'd', '!'};

    busywait_approx_1s();
    ntw_transmit(app_payload, 13);
